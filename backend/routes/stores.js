...
router.post('/', [auth, adminOnly], async (req, res) => {
    const { name, address, owner_id } = req.body; // owner_id is optional
    try {
...
// Normal User & Admin: View all stores with search
...
router.post('/:storeId/ratings', auth, async (req, res) => {
    ...
    try {
        // Upsert logic: INSERT a new rating or UPDATE the existing one
        const query = `
            ...
            RETURNING *;
        `;
        const { rows } = await db.query(query, [userId, storeId, rating]);
...
router.get('/owner/dashboard', auth, async (req, res) => {
    ...
    try {
        ...
        const ratersResult = await db.query(
            'SELECT u.name, u.email FROM users u JOIN ratings r ON u.id = r.user_id WHERE r.store_id = $1',
            [storeId]
        );
